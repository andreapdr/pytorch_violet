...
class EncImg(T.nn.Module):
    def __init__(self):
        ...
        self.swin.load_state_dict(
            # T.load("./_snapshot/ckpt_video-swin.pt", map_location="cpu")
            T.load(
                "models/video/pytorch_violet/_snapshot/ckpt_video-swin.pt",
                map_location="cpu",
            )
        )
    ...
    def forward(self, img):
        ...
        img = TV.transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])(img)
...
class VIOLET_Base(T.nn.Module):
    ...
    def go_cross(self, feat_img, mask_img, feat_txt, mask_txt):
        feat, mask = T.cat([feat_img, feat_txt], dim=1), T.cat(
            [mask_img, mask_txt], dim=1
        )
